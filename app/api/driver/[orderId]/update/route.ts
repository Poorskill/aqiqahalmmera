  ...
  try {
    ...
    if (action === 'complete' || action === 'delivered') {
      ...
      if (proofFile && proofFile.size > 0) {
        ...
        deliveryProof = await uploadFile('proofs', `delivery-proofs/${orderId}/${crypto.randomUUID()}${ext}`, proofFile);
      }
      ...
      try {
        await completePostgresDelivery(orderId, user.id, deliveryProof, deliveryNote);
      } catch {
